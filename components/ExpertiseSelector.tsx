              ...
              {(Object.entries(STATE_LABELS) as [ExpertiseState, string][]).map(([state, label]) => (
                <Pressable
                  ...
                  style={[
                    ...
                    value[activePart!] === state && { backgroundColor: colors.tintLight }
                  ]}
                >
                  <View style={styles.stateItemLeft}>
                    <View style={[styles.stateDot, { backgroundColor: STATE_COLORS[state] }]} />
                    <Text style={[styles.stateLabel, { color: value[activePart!] === state ? colors.tint : colors.text }]}>
                      {label}
                    </Text>
                  </View>
                  {value[activePart!] === state && <Ionicons name="checkmark" size={20} color={colors.tint} />}
                </Pressable>
              ))}
            </View>
          </View>
